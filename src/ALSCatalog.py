...
def generate_catalog(catalog_cache_file, luts_set, smt_timeout, solver):
	...
	for lut_conf in luts_set:
		...
		if exact_spec is None:
			...
			while gates > 0:
				synt_spec, S, P, out_p, out, depth = synth_spec(lut_conf, hamming_distance, solver, smt_timeout)
				print(f"{lut_conf}@{hamming_distance} synthesized as {synth_spec} using {len(S[0])} gates at depth {depth}.")
				if len(S[0]) < gates:
					lut_specifications.append({"spec": synt_spec, "gates": len(S[0]), "S": S, "P": P, "out_p": out_p, "out": out, "depth": depth})
					cache.add_lut(lut_conf, hamming_distance, synth_spec, S, P, out_p, out, depth)
				gates = len(S[0])
				hamming_distance += 1
		else:
			lut_specifications = [exact_spec] + cache.get_approx_luts(lut_conf)
		catalog.append(lut_specifications)
	return catalog


def synth_spec(lut_spec, dist, solver, es_timeout):
